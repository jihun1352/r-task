...
public class MemberController {
	...
	// 로그인
	@PostMapping("/login")
	public String loginf(HttpServletRequest request, MemberDTO memberDto, RedirectAttributes redirectAttributes) {
		
		if(!memberService.login(memberDto)) {
			redirectAttributes.addFlashAttribute("message", "아이디가 없거나 비밀번호가 일치하지 않습니다.");
			return "redirect:/login";
		}
		request.getSession().setAttribute("user_id", memberDto.getUserId());
	...
	// 회원가입
	@PostMapping("/join")
	public String joinf(MemberDTO memberDto, RedirectAttributes redirectAttributes) {
